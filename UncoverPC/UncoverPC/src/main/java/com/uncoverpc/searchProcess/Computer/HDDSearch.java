...
public class HDDSearch extends SearchComponent implements Runnable {
	 ...
	 */
	public HDDSearch(PC pc, boolean searchAmazon, boolean searchCanadaComputer, boolean searchNewegg) {
		super(pc, searchAmazon, searchCanadaComputer, searchNewegg);
	}

	/**
	 * Searches for exact and similar hardrive as the prebuilt
	 */
	public void run() {
		try {
			// Getting Hard drive
			String HDD = pc.getHddSize().concat(" 7200 rpm");
			//Searching
			searchComponents(HDD);
			
			// Checking if there is an avaliable HDD of similar type
			...
			System.err.println("Error Searching for Hard drive");
			e.printStackTrace();
		}
	}
	...
	public Component getAndSetRelativeComponent(ArrayList<Component> results) throws Exception {
		...
		for (Component result : results) {
			// Checking if result is a hdd
			if (result.getTitle().toLowerCase().contains(" 7200 rpm".toLowerCase())) {
				// Checking if it is desired price
				// These numbers are based of research of Total PC cost to HDD cost ratio
